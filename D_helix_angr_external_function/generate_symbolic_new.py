...
def main_each_function_klee(i,function_name_list,filename,filepath_originalclang,concrete_input_num):
    ...
    if os.path.isfile(filepath_generatedbc)  :
        ...
        if foreverloop == False:
            print("\ntimeout 30s /home/friends/muqi/PROMPT/build3/bin/klee  -prose-api-model="+filepath_prompt_model_on_function+ " --search=bfs --loop-bound=2 --solver-backend=z3 --posix-runtime " + filepath_generatedbc + " > "+ filepath_log_klee + " 2> "+ filepath_log_klee_error)
        else:
            print("\ntimeout 30s /home/friends/muqi/PROMPT/build2/bin/klee  -prose-api-model="+filepath_prompt_model_on_function+ " --search=bfs  --solver-backend=z3 --posix-runtime " + filepath_generatedbc + " > "+ filepath_log_klee + " 2> "+ filepath_log_klee_error)
    
        if foreverloop == False:
            run_cmd("/home/friends/muqi/PROMPT/build3/bin/klee -prose-api-model="+filepath_prompt_model_on_function+ "  --loop-bound=2  --search=bfs --solver-backend=z3 --posix-runtime " + filepath_generatedbc + " > "+ filepath_log_klee + ' 2> '+ filepath_log_klee_error,120)
        else:
            run_cmd("/home/friends/muqi/PROMPT/build2/bin/klee -prose-api-model="+filepath_prompt_model_on_function+ "   --search=bfs --solver-backend=z3 --posix-runtime " + filepath_generatedbc + " > "+ filepath_log_klee + ' 2> '+ filepath_log_klee_error,5)
        #os.system("rm /tmp/klee_"+filename+"_"+function_name[:-1] +"_external_input.txt")
        os.system("rm /tmp/klee_"+filename+"_"+function_name[:-1] +"_external_input_bool.txt")
...
def correct_angr_numargs_using_ghidra(angr_args,filename,filepath_folder_for_project):
    ...
    for i in range(len(binary_function_call)):
        ...
        function_name_brack = func_addr_num[1]+"("
        print(function_name_brack)
        
        file_function_name = filename +"_"+ func_addr_num[0]
        try:
            print(filepath_folder_for_project+"/"+file_function_name+"_modified.c")
            ghidra_code_file =  open(filepath_folder_for_project+"/"+file_function_name+"_modified.c","r")
        ...
        while j < len(ghidra_code):
            if function_name_brack in ghidra_code[j] :
            
                start_index = j
                end_index = start_index
                print(start_index)
                
                while end_index < len(ghidra_code):
                    #we do not care how ghidra declare this function
                    if ")" in ghidra_code[end_index] and "{" in ghidra_code[end_index+2]:
                        j = end_index+2
                        start_index = 0
                        end_index = 0
                        break
        ...
        if function_name_brack+");" in ghidra_code[start_index] :
            num_of_args_ghidra = 0
        else:
            num_of_args_ghidra = 1
            addup = 0
            for k in range(start_index,end_index+1):
                addup += ghidra_code[k].count(',')
            num_of_args_ghidra += addup

        if func_addr_num[3] != str(num_of_args_ghidra):
            print("error about angr")
            print(func_addr_num)
            
        write_binary_file.write(func_addr_num[0]+"|"+func_addr_num[1]+"|"+
                        func_addr_num[2]+
                        "|"+str(num_of_args_ghidra) +"\n")
...
def main_each_program(filename):
    ...
    correct_angr_numargs_using_ghidra("/tmp/call_count_"+filename+"_angr_numargs.txt",filename,filepath_folder_for_project)
