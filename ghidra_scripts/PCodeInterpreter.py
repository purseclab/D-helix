...
def checkFixParameters(func, parameters):
	...
	#func_proto = hf.getFunctionPrototype()
        local_symbol_map = hf.getLocalSymbolMap()
	if local_symbol_map.getNumParams() != len(parameters) and not func.hasVarArgs():
		print(func, "call signature wrong...")
		raise Exception("Function call signature different")

	argument_varnodes = []
	for i in range(local_symbol_map.getNumParams()):
                print("type of local_symbol_map",local_symbol_map) 
                print("type of local_symbol_map.getParam(i)",local_symbol_map.getParam(i))
		cur = local_symbol_map.getParam(i).getRepresentative()
...
def analyzeFunctionBackward(func, pci, init_param=None):
	...
	for i in range(local_symbol_map.getNumParams()):
                print("type of local_symbol_map",local_symbol_map)                
                print("type of local_symbol_map.getParam(i)",local_symbol_map.getParam(i))
		argument_varnodes.append(local_symbol_map.getParam(i).getRepresentative())
...
def analyzeFunctionForward(func, pci):
	...
	hf = get_highfunction(func)
	HighFunctionDBUtil.commitParamsToDatabase(hf, True, SourceType.DEFAULT)
        print("in forward analyze")
	print("paramters are:", func.getParameters(),"num Parameters are", hf.getLocalSymbolMap().getNumParams())
	...
	argument_nodes = []
	for i in range(local_symbol_map.getNumParams()):
                print("type of func_proto:",local_symbol_map)
                print("type of func_proto.getParam(i):",local_symbol_map.getParam(i))
                print("type of parameters():", func.getParameters())
		argument_varnodes.append(local_symbol_map.getParam(i).getRepresentative())
