...
def find_functionargs(filepath_generated_whole_c,filename_c):
    ...
    for i in range(len(function_name_list)):
        ...
        while k < len(originalc_linelist):
            if (  function_name_list[i].replace(".","_") in originalc_linelist[k][0:len(function_name_list[i])] and "(" in  originalc_linelist[k+1]) or ( function_name_list[i].replace(".","_")+"(" in originalc_linelist[k][0:len(function_name_list[i])+1])or (" "+function_name_list[i].replace(".","_")+"(" in originalc_linelist[k]) or ( " "+function_name_list[i].replace(".","_")+"\n" in originalc_linelist[k] and "(" in originalc_linelist[k+1] ) :
...
def main_each_function_klee(i,function_name_list,filename,filepath_originalclang):
    ...
    if os.path.isfile(filepath_generatedbc)  :
        ...
        run_cmd("/home/muqi/PROMPT/build/bin/klee  -prose-api-model="+filepath_prompt_model_on_function+ " --search=bfs --solver-backend=z3 --posix-runtime " + filepath_generatedbc + ' 1> '+ filepath_log_klee +' 2> '+ filepath_log_klee_error, 180)

#@func_set_timeout(3)
def main_each_function_angr(i,function_name_list,filename,pcfg_angr,angr_project,filepath_originalclang):
    @atexit.register
    def goodbye():
        print("Time to kill this thread")
    ...
    if os.path.isfile(filepath_generatedbc)  :
        ...
        #we cannot do this because we cannot pass angr_project argument.
            #os.system("timeout 300s python angr_running_pointer.py filepath_originalclang,function_name[:-1],angr_project,pcfg_angr,filename")
        
        #not work in multiple threading
        '''
        ...
        #not work under multiple process
        '''
        try:
            if klee_work == True:
                #angr_running_pointer(filepath_originalclang,function_name[:-1],angr_project,pcfg_angr,filename)
                ...
                #print("running angr running\n")
                process_angr_running = multiprocessing.Process(target=angr_running_pointer, name="angr_running_pointer", args=(filepath_originalclang,function_name[:-1],angr_project,pcfg_angr,filename,))
...
def main_each_program(filename):
    ...
    #for i in range(len(function_name_list)):
    '''#this is slow because for multiple processes, we need to copy and paste the arguments, which is super large
    pool = Pool()
    args = ((i,function_name_list,filename,pcfg_angr,angr_project,filepath_originalclang) for i in range(len(function_name_list)))
    pool.starmap(main_each_function2,args)
    pool.close()
    pool.join()
    '''
    ...
    #we do not use this. Since in multi thread, sub process will be killed by other threads.
    '''
    Thread_list = []
    for i in range(len(function_name_list)):
        Thread_list.append( threading.Thread(target = main_each_function_klee, args = (i,function_name_list,filename,filepath_originalclang) ))
    ...
    for i in range(len(function_name_list)):
        #e = threading.Event()
        t = threading.Thread(target = main_each_function_angr, args = (i,function_name_list,filename,pcfg_angr,angr_project,filepath_originalclang) )
        #Thread_list[i].daemon = True
        t.setDaemon(True)
        t.start()
        ...
        #e.set()
        #t.join()
        #Thread_list[i].join(timeout=660)
    '''
...
def main():
    ...
    #'''
    '''
    for i in os.listdir(directname_originalclang):
        main_each_program(i)
    '''
    #os.system("clear")
