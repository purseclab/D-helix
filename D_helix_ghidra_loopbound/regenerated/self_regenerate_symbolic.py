...
directname_function_name_original = "./function_name_original_individual"
...
def automatic_compilation(arg1,arg2,function_name,filepath_individual_function_log,filepath_individual_function_log):
    #print(arg1)
...
def seperate_function_compilation(filepath_generated_whole_c,filename_c):
    ...
    for i in range(len(function_name_list)):
        ...
        while k < len(originalc_linelist):
            if function_name_list[i].replace(".","_")+"(" in originalc_linelist[k] or ( function_name_list[i].replace(".","_") in originalc_linelist[k] and "(" in originalc_linelist[k+1] ) :
        ...
        try:
            automatic_compilation(filepath_individual_function,filepath_individual_function_bc,function_name_list[i],filepath_individual_function_log)
        except:
            pass
...
def angr_running_pointer(arg1,arg2,angr_project,pcfg_input,filename):
    ...
    state = p.factory.call_state(log_filepath, required_address,args[0],args[1],args[2],args[3],args[4],args[5],args[6],args[7],args[8],args[9],args[10],add_options={angr.options.CALLLESS,angr.options.STRINGS_ANALYSIS,angr.options.ZERO_FILL_UNCONSTRAINED_MEMORY,angr.options.ZERO_FILL_UNCONSTRAINED_REGISTERS})
...
def create_and_copy_correct_function_name(filepath_originalclang,filename_path,filename_path_original,filename):
    ghidra_repo_address = ghidra_repo_address_folder+ filename+"_name.gpr"
    os.system(ghidra_address+" "+ghidra_repo_address+" -import " + filepath_originalclang + " -overwrite -scriptPath " + ghidra_decompiled_java_script1 +" -prescript "+ ghidra_decompiled_java_script3 + " -postscript " + ghidra_get_function_name_java_script+" "+filename_path +" ")
...
def permutate_run_ghidra(filepath_originalclang,filepath_generated_whole_c,filename):
    ghidra_repo_address = ghidra_repo_address_folder+ filename+"_original.gpr"
    print(ghidra_address+" "+ghidra_repo_address+" -import " + filepath_originalclang + " -overwrite -scriptPath " + ghidra_decompiled_java_script1 +" -prescript "+ ghidra_decompiled_java_script3 + " -postscript " + ghidra_decompiled_java_script2+" "+filepath_generated_whole_c +" ")
    os.system(ghidra_address+" "+ghidra_repo_address+" -import " + filepath_originalclang + " -overwrite -scriptPath " + ghidra_decompiled_java_script1 +" -prescript "+ ghidra_decompiled_java_script3 + " -postscript " + ghidra_decompiled_java_script2+" "+filepath_generated_whole_c +" ")
...
def main():
    ...
    #os.system("python cleanall_onetime.py")
    #os.system("cp ../test_muqi/originalclang/* ./test_muqi/originalclang/")
    ...
    for filename in os.listdir(directname_originalclang):
        ...
        '''
        permutate_run_ghidra(filepath_originalclang,filepath_generated_whole_c,filename_clang)
        create_and_copy_correct_function_name(filepath_originalclang,filepath_generated_function_name,filepath_original_function_name,filename_clang)
        remove_duplicated_function_name(filepath_generated_function_name)
        '''
        #continue
        
        #'''
        t1 = threading.Thread(target = permutate_run_ghidra, args = (filepath_originalclang,filepath_generated_whole_c,filename_clang,))
        t2 = threading.Thread(target = create_and_copy_correct_function_name, args = (filepath_originalclang,filepath_generated_function_name,filepath_original_function_name,filename_clang,))
        ...
        try:
            ...
            f.write(filename+": "+function_name + " is wrong during cfg analyzing!\n")
            f.close()
        #'''
