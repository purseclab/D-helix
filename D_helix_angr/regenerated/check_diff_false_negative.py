...
def check_empty(directname_function_content,individual_functionname):
    ...
    try:
        ...
        for i in range(len(linelist)-3):
            if individual_functionname in linelist[i] and  "{" in linelist[i+1] and "}" in linelist[i+2]:
                return True
        return False
        #'''
    except:
...
def write_log_to_file(fout,linelistdiff,linelistz3,filename):
    ...
    '''
    empty_or_not = check_empty(directname_function_content,individual_functionname)
    if (empty_or_not == True):
        os.system("cp "+directname_function_content+" ./empty_function")
