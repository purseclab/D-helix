...
def automatic_compilation(arg1,arg2,function_name,filepath_individual_function_log,generated_html_filepath):
    ...
    fout.write("typedef long long    longlong;\n")
    ...
    max_iteration = 1
    #'''/home/muqi/llvm-3.8/llvm-src/tool/clang/cfe-3.8.0.src/build/bin/clang -emit-llvm -O0  -c -Wl,--demangle -Wno-everything 
    ...
    while len(loglist) != 0 and max_iteration > 0:
        ...
        os.system(compiler + " -emit-llvm -O0  -c -Wl,--demangle -Wno-everything " + filepath_generatedc + " -o " + filepath_generatedbc +"  2> " + filepath_individual_function_log)
...
def seperate_function_compilation(filepath_generated_whole_c,filename_c,filepath_generated_html):
    ...
    variable_count = 0
    first_function_name = first_function_name.replace("-fwdslash-","/")
    fout = open(filepath_basec,'w')
    for i in range(len(originalc_linelist)-1):
        if first_function_name in originalc_linelist[i+1]:
            if "Original name:" in  originalc_linelist[i+1] or  "Unable to decompile" in originalc_linelist[i+1]:
               fout.write("\n")
               break
        ...
        #for Windows binary
        if "+" in  originalc_linelist[i] and "struct" in  originalc_linelist[i] and ";" in  originalc_linelist[i] :
             originalc_linelist[i] = "//"+originalc_linelist[i] 
        if "struct" in originalc_linelist[i] and "UnwindCode;" in originalc_linelist[i] :
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "typedef unsigned long long    undefined8" in  originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "pointer32" in originalc_linelist[i] and ";" in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "pointer64" in  originalc_linelist[i] and ";" in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "typedef struct _iobuf FILE" in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "typedef" in originalc_linelist[i] and "size_t" in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "pointer64 ImageBase" in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "UnwindOpCode:4;" in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "typedef long long    longlong;" in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "typedef longlong __time64_t;" in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        #if "typedef longlong LONGLONG;" in originalc_linelist[i]:
        #     originalc_linelist[i] = "//"+originalc_linelist[i]
        if "typedef char * va_list;" in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "typedef __time64_t time_t;" in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "IMAGE_DATA_DIRECTORY.conflict" in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "CLI_Stream_Header_#" in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "typedef ulong __uint32_t;" in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "typedef longlong intptr_t;" in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "typedef struct WriteOnlyArray" in originalc_linelist[i] and ";" in originalc_linelist[i] :
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "struct WriteOnlyArray<class_Platform::String_^___ptr64,1>" in originalc_linelist[i]:
             originalc_linelist[i] = originalc_linelist[i].replace("<class_Platform::String_^___ptr64,1>","")
        if "typedef struct function<void___cdecl(void)>" in  originalc_linelist[i] and ";" in originalc_linelist[i] :
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "function<void___cdecl(void)>" in originalc_linelist[i]:
             originalc_linelist[i] = originalc_linelist[i].replace("<void___cdecl(void)>","")
        if "typedef struct _Yarn<char>" in  originalc_linelist[i] and ";" in originalc_linelist[i] :
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "struct _Yarn<char>" in originalc_linelist[i]:
             originalc_linelist[i] = originalc_linelist[i].replace("<char>","")
        if "typedef struct ctype<wchar_t>" in  originalc_linelist[i] and ";" in originalc_linelist[i] :
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "struct ctype<wchar_t>" in originalc_linelist[i]:
             originalc_linelist[i] = originalc_linelist[i].replace("<wchar_t>","")
        if "typedef enum error_type {" in originalc_linelist[i] and "} error_type;" in  originalc_linelist[i+1] :
             originalc_linelist[i] = "//"+originalc_linelist[i]
             originalc_linelist[i+1] = "//"+originalc_linelist[i+1]
        if "typedef enum TypeCode {" in originalc_linelist[i] and "} TypeCode;" in  originalc_linelist[i+1] :
             originalc_linelist[i] = "//"+originalc_linelist[i]
             originalc_linelist[i+1] = "//"+originalc_linelist[i+1] 
        if "float10"  in originalc_linelist[i][0:9]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "typedef longlong fpos_t;"  in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "typedef int intptr_t;"  in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "typedef uint uintptr_t;" in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "typedef struct IMAGE_SECTION_HEADER IMAGE_SECTION_HEADER, *PIMAGE_SECTION_HEADER;" in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        #for macos
        if "typedef ulonglong __uint64_t;"  in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "typedef longlong __int64_t;" in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "typedef union __mbstate_t __mbstate_t, *P__mbstate_t;" in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if " blksize_t st_blksize;" in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "typedef __darwin_off_t fpos_t;"  in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "typedef struct __sFILE FILE;" in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "typedef struct Unsupported Load Command Type" in originalc_linelist[i]:
             originalc_linelist[i] = "//"+originalc_linelist[i]
        if "struct Unsupported Load Command Type = " in originalc_linelist[i] and "{" in originalc_linelist[i] :
             originalc_linelist[i] = "//"+originalc_linelist[i]
             originalc_linelist[i+1] = "//"+originalc_linelist[i+1]
             originalc_linelist[i+2] = "//"+originalc_linelist[i+2]
             originalc_linelist[i+3] = "//"+originalc_linelist[i+3]
        #comment out FS_OFFSET
    ...
    for i in range(len(originalc_linelist)):
        originalc_linelist[i]= originalc_linelist[i].replace("__stubs::","").replace("*(code *)__got::","")
    ...
    for i in range(len(function_name_list)):
        ...
        if os.path.isfile(filepath_individual_function_bc) :
            continue
        ...
        while k < len(originalc_linelist):
            ...
            if "{" in originalc_linelist[k] and "{ // try" not in originalc_linelist[k] :    
            #if "{" in originalc_linelist[k] :
                braket_count += 1
    ...
    try :
        ...
        with open(filepath_objdump,'r') as objdump_file:
            ...
            for i in range(len(objdump_lines)):
                if Macos == True:
                   required_function_check = required_function
                else:
                   required_function_check = required_function+"."
                if required_function_check in objdump_lines[i] :
...
def generate_function_name(filepath_originalclang,filepath_generated_function_name,filename):
    ...
    if ARM_enable == True:    
        os.system(ghidra_address+" "+ghidra_repo_address+" -import " + filepath_originalclang + " -processor \"ARM:LE:32:v8T\"  -overwrite -postscript " + ghidra_get_function_name_java_script+" "+filepath_generated_function_name +" ")
    else:
...
def generate_function_whole_c(filepath_originalclang,filepath_generated_whole_c,filename):
    ...
    if ARM_enable == True:
        os.system(ghidra_address+" "+ghidra_repo_address+" -import " + filepath_originalclang + " -processor \"ARM:LE:32:v8T\" -overwrite -scriptPath " + ghidra_decompiled_java_script1 +" -postscript " + ghidra_decompiled_java_script2+" "+filepath_generated_whole_c +" ")
    else:
...
def generate_function_html(filepath_originalclang,filepath_generated_html,filename):
    ...
    if ARM_enable == True:
        os.system(ghidra_address+" "+ghidra_repo_address+" -import " + filepath_originalclang + " -processor \"ARM:LE:32:v8T\" -overwrite -scriptPath " + ghidra_decompiled_java_script1 +" -postscript " + ghidra_decompiled_java_script3+" "+filepath_generated_html +" ")
    else:
...
def main_each_function_klee(i,function_name_list,filename,filepath_originalclang):
    ...
    if os.path.isfile(filepath_generatedbc)  :
        ...
        #'''
        if os.path.isfile(filepath_log_klee) :
            pass
        #'''
        '''
        try:
...
def main_each_function_angr(i,function_name_list,filename,pcfg_angr,angr_project,filepath_originalclang):
    ...
    print("after os.path.isfile(z3_filepath)")
    print(filepath_generatedbc)
    print(os.path.isfile(filepath_generatedbc))
    if os.path.isfile(filepath_generatedbc) and os.path.isfile(filepath_modelprompt) :
        ...
        filepath_log_klee = os.path.join(directname_log_klee,log_klee_function_name)
        print("before reading log_klee"+filename+function_name[:-1])
        ...
        try:
            ...
            f.close()
        '''
        print("end of reading log_klee"+filename+function_name[:-1])

        if klee_work == True:
            print("before angr_running_pointer"+filename+function_name[:-1])
        ...
        print("end of angr_running_pointer")
        #'''
        #start analyzing
        if klee_work == True and angr_work == True:
            ...
            '''
            print("before angr analyze"+filename+function_name[:-1])
            try:
                log_filepath_no_suffix='/tmp/angr_'+filename +'_'+ function_name[:-1] 
                angr_log = log_filepath_no_suffix+".txt"
                angr_ir_first = log_filepath_no_suffix+ "_ir_first.txt"
            ...
            print("end angr analyze"+filename+function_name[:-1])
            #generate klee result
            #analyze_results.analyze_results(i,filename,function_name)
            #'''
            if angr_log_work == True:
                print("before klee analyze"+filename+function_name[:-1])
                try: 
                    analyze_results.analyze_results(i,filename,function_name)
                except:
                    klee_log_work = False 
...
def main_each_program(filename):
    ...
    print("before reading namelist"+filepath_generated_function_name)
    try:
    ...
    #'''
    print("before main_each_function_klee" +filepath_generated_function_name)

    #return
    #'''
    for i in range(len(function_name_list)):
        print("in main_each_function_klee"+filename+str(i))
        main_each_function_klee(i,function_name_list,filename,filepath_originalclang)
    #'''
    #return

    #'''
    print("before find_functionargs"+filename_c)
    find_functionargs(filepath_generated_whole_c,filename_c)
    print("after find_functionargs"+filename_c)
    ...
    try:
        ...
        if ARM_enable == True:
            angr_project = angr.Project(filepath_originalclang, main_opts={'backend': 'blob', 'arch': 'arm'}, auto_load_libs=False, load_debug_info=True)
        else:
        ...
        #pcfg_angr = angr_project.analyses.CFGEmulated()
    except:
        f = open(angrlog_file, "a")
    ...
    if (len(function_name_list)) > 0 :
        for i in range(len(function_name_list)):
            print("in main_each_function_angr" + function_name_list[i] +filename)
            try:
...
def z3_each_file(filename):
    ...
    filepath_z3_fourth_cfgfast = os.path.join(directname_z3_fourth_cfgfast,filename)
