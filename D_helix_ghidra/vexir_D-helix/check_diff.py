...
def write_log_to_file(fout,linelistdiff,linelistz3,filename):
    ...
    filename_forfile = "Unknown"
    print(whole_functionname)
    os.system("find ../test_muqi/generated_function_c/project_folder/ -name " + whole_functionname+"_modified.c > temp_found_file.txt")
    ...
    if (print_or_not == False):
        ...
        start_line = 0
        try:
            fc_out = open(directname_function_content,'r')
            fc_content = fc_out.readlines()
            fc_out.close()
            for fc_i in range(len(fc_content)-1):
                if individual_functionname in fc_content[fc_i]:
                    start_line = fc_i
                    break
        except:
            pass
        ...
        for tmp in range(start_line, len(fc_content)-1):
            fout.write(fc_content[tmp])
...
def main():
    ...
    for filename in os.listdir(directname_diff):
        ...
        if len(linelistdiff) > 1 :
            #for win. too many parameter mismatch
